...
# ----------------------------------------
# Selection of the best features with their scores
# ----------------------------------------
...
# -------
# Make a local copy of features_list and remove the label 
# from it. That gives us the list of features that we passed 
# to the estimators in the pipleline
# -------
flist = features_list[:]
flist.remove('poi')
len(flist)
...
for i0 in range(no_of_pcs):
    for i1 in range(no_of_pcs):
        if i0 < i1:
           ...
           #if abs(sum_of_scalar_prod) < .000000000000001:
